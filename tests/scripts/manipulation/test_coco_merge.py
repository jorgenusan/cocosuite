...
def test_coco_merge_with_example_files(project_root_path):
    ...
    expected_num_annotations = len(data1.get("annotations", [])) + len(
        data2.get("annotations", [])
    )
    expected_num_categories = len(
        {
            cat["name"]
            for cat in data1.get("categories", []) + data2.get("categories", [])
        }
    )
