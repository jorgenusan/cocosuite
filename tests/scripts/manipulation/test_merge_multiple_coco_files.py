...
from cocosuite.scripts.manipulation.merge_multiple_coco_files import add_ann_folder_to_img_name, merge_multiple_coco_files, delete_merged_files
...
def test_merge_multiple_coco_files_integration(project_root_path):
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        merge_multiple_coco_files(str(temp_dir_path), output_file="merged_annotations.json", name_pattern="*.json")

        result_file = temp_dir_path / "merged_annotations.json"
        assert result_file.exists()

        merged_data = load_json_file(result_file)
        expected_num_images = sum(len(load_json_file(file)["images"]) for file in example_files)
        expected_num_annotations = sum(len(load_json_file(file)["annotations"]) for file in example_files)
        unique_categories = set()
        for file in example_files:
            data = load_json_file(file)
            unique_categories.update(category["name"] for category in data["categories"])
